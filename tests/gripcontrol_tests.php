...
class GripControlTestClass extends GripControl
{
...
class GripControlTests extends PHPUnit_Framework_TestCase
{
    ...
    {
        GripControl::create_hold('mode', array(), 'response');
    }

    public function testCreateHold()
    {
        $hold = json_decode(GripControl::create_hold('mode', 'channel',
                new Response('code', 'reason', 'headers', 'body')));
    ...
    public function testValidateSig()
    {
        ...
        $token = JWT::encode(array('iss' => 'realm', 'exp' => time() - 3600),
                'key');
        $this->assertEquals(GripControl::validate_sig($token, 'key'), false);
        $token = JWT::encode(array('iss' => 'realm', 'exp' => time() + 3600),
                'key');
        $this->assertEquals(GripControl::validate_sig($token, 'wrong_key'), false);
    }
    ...
    {
        GripControl::create_grip_channel_header(array());
    }
    ...
    public function testCreateGripChannelHeader()
    {
        ...
        $header = GripControl::create_grip_channel_header(new Channel('channel'));
        $this->assertEquals($header, 'channel');
        $header = GripControl::create_grip_channel_header(new Channel('channel',
                'prev-id'));
        $this->assertEquals($header, 'channel; prev-id=prev-id');
        $header = GripControl::create_grip_channel_header(array(
    ...
    {
        $hold = json_decode(GripControl::create_hold_response('channel',
                new Response('code', 'reason', 'headers', 'body')), true);
        ...
        $hold = json_decode(GripControl::create_hold_response('channel', null,
                'timeout'), true);
    ...
    {
        $hold = json_decode(GripControl::create_hold_stream('channel', new Response(
        ...
        $this->assertEquals($hold['response'], array('code' => 'code',
                'reason' => 'reason', 'headers' => 'headers', 'body' => 'body'));
        $hold = json_decode(GripControl::create_hold_stream('channel', null), true);
        $this->assertFalse(array_key_exists('response', $hold));
        $this->assertEquals($hold['hold']['mode'], 'stream');
    }
    ...
    {  
        $events = GripControl::encode_websocket_events(array(
    ...
    public function testWebSocketControlMessage()
    {
        ...
        $message = json_decode(GripControl::websocket_control_message(
    ...
    public function testGetHoldChannels()
    {
        ...
        $this->assertEquals($hold_channels[0], array('name' => 'channel', 'prev-id' =>
                'prev-id'));
        $hold_channels = GripControlTestClass::callGetHoldChannels(array(
                new Channel('channel1', 'prev-id1'),
                new Channel('channel2', 'prev-id2')));
    ...
    public function testGetHoldResponse()
    {
        ...
        $response = GripControlTestClass::callGetHoldResponse(new Response('code', 'reason',
        ...
        $this->assertEquals($response['body'], "body\u2713");
        $response = GripControlTestClass::callGetHoldResponse(new Response(
